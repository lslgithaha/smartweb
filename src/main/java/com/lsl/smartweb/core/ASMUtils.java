...
public class ASMUtils {
     ...
     * @throws IOException
     */
    public static String[] getMethodParamNames(Method m) throws IOException {
        Class<?> clazz = m.getDeclaringClass();
        try (InputStream in = clazz.getResourceAsStream("/" + clazz.getName().replace('.', '/') + ".class")) {
            return getMethodParamNames(in,m).toArray(new String[0]);
        }
    ...
    private static List<String> getParamNames(InputStream in, EnclosingMetadata m) throws IOException {
        ...
        cr.accept((ClassVisitor) cn, ClassReader.EXPAND_FRAMES);// 建议EXPAND_FRAMES
        // ASM树接口形式访问
